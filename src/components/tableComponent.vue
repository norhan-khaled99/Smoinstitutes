    ...
    <div class="table_content">
      <div class="row">
        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
          <q-table :columns="tableColumns" :rows="tableRows" v-model:pagination="pagination" @request="onRequest">
            <!--Start pagination  -->
            <template v-slot:pagination v-if="displayPagination">
              <div class="row justify-between full-width items-center pagination-wrapper">
                ...
                <div class="pagination-controls">
                  <q-pagination v-model="pagination.page" :max="pagesNumber" :max-pages="5" direction-links
                    @update:model-value="getPagination(pagination)" class="custom-pagination">
                    <template v-slot:prev>
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M15 19L9 12L15 5" stroke="#666D80" stroke-width="1.5" stroke-linecap="round"
                          stroke-linejoin="round" />
                      </svg>
                    </template>
                    <template v-slot:next>
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M9 19L15 12L9 5" stroke="#666D80" stroke-width="1.5" stroke-linecap="round"
                          stroke-linejoin="round" />
                      </svg>
                    </template>
                  </q-pagination>
            ...
            <template v-slot:top-left>
              <div class="filter-container row justify-between items-center full-width">
                <div class="row filter-bar items-center">
                  <div class="search_table" v-if="searchInput">
                    <q-input outlined @keyup.enter="onSearch" v-model="searchResult" dense
                  ...
                  <div class="filter-bar row items-center wrap" v-if="showFilters">
                    <div class="filter-dropdowns row items-center wrap">
                      ...
                      <div class="filter-item-wrapper" v-if="transactions">
                        <q-input outlined v-model="fromNo" dense placeholder="From No." class="filter-input"
                          @update:model-value="onFilterChange('fromNo', fromNo)" />
                      </div>
                      <div class="filter-item-wrapper" v-if="transactions">
                        <q-input outlined v-model="toNo" dense placeholder="To No." class="filter-input"
                      ...
                      <div class="filter-item-wrapper" v-if="showStatusFilter">
                        <q-select outlined v-model="statusFilter" dense :options="statusOptions" option-label="name"
                          option-value="id" fill-input emit-value map-options use-input hide-selected input-debounce="0"
                      ...
                      <div class="filter-item-wrapper" v-if="showNoteTypeFilter">
                        <q-select outlined v-model="noteTypeFilter" dense :options="noteTypeOptions" option-label="name"
                          option-value="id" fill-input emit-value map-options use-input input-debounce="0"
                          class="filter-select" :placeholder="noteTypeFilter ? '' : 'Note type'" @update:model-value="
                            onFilterChange('noteType', noteTypeFilter)
                            ">
                          <template v-slot:append>
                            <q-icon v-if="noteTypeFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                              noteTypeFilter = null;
                      ...
                      <div class="filter-item-wrapper" v-if="showCreatedAtFilter">
                        <q-select outlined v-model="createdAtFilter" dense :options="createdAtOptions"
                          option-label="name" option-value="id" fill-input emit-value map-options use-input
                          input-debounce="0" class="filter-select" :placeholder="createdAtFilter ? '' : 'Created at'"
                          @update:model-value="
                            onFilterChange('createdAt', createdAtFilter)
                            ">
                          <template v-slot:append>
                            <q-icon v-if="createdAtFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                      ...
                      <div class="filter-item-wrapper" v-if="showCreatedByFilter">
                        <q-select outlined v-model="createdByFilter" dense :options="createdByOptions"
                          option-label="name" option-value="id" fill-input emit-value map-options use-input
                          input-debounce="0" class="filter-select" :placeholder="createdByFilter ? '' : 'Created by'"
                          @update:model-value="
                            onFilterChange('createdBy', createdByFilter)
                            ">
                          <template v-slot:append>
                            <q-icon v-if="createdByFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                      ...
                      <div class="filter-item-wrapper" v-if="showDirectionFilter">
                        <q-select outlined v-model="directionFilter" dense :options="directionOptions"
                          option-label="name" option-value="id" emit-value map-options class="filter-select"
                          :label="directionFilter ? '' : 'Direction'" @update:model-value="
                            onFilterChange('direction', directionFilter)
                            ">
                          <template v-slot:append>
                            <q-icon v-if="directionFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                      ...
                      <div class="filter-item-wrapper" v-if="showLevelFilter">
                        <q-select outlined v-model="levelFilter" dense :options="levelOptions" option-label="levelname"
                          option-value="id" fill-input emit-value map-options use-input input-debounce="0"
                          class="filter-select" :placeholder="levelFilter ? '' : 'Level'" @update:model-value="
                            onFilterChange('level_id', levelFilter)
                            ">
                          <template v-slot:append>
                            <q-icon v-if="levelFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                              levelFilter = null;
                      ...
                      <div class="filter-item-wrapper" v-if="showByUserFilter">
                        <q-select outlined v-model="byUserFilter" dense :options="byUserOptions" option-label="username"
                          option-value="id" fill-input emit-value map-options use-input input-debounce="0"
                          class="filter-select" :placeholder="byUserFilter ? '' : 'By user'" @update:model-value="
                            onFilterChange('by_user', byUserFilter)
                            ">

                          <template v-slot:append>
                            <q-icon v-if="byUserFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                              byUserFilter = null;
                      ...
                      <div class="filter-item-wrapper" v-if="showTeacherFilter">
                        <q-select outlined v-model="teacherFilter" dense :options="teacherOptions" option-label="name"
                          option-value="id" fill-input emit-value map-options use-input input-debounce="0"
                          class="filter-select" :placeholder="teacherFilter ? '' : 'Teacher'" @update:model-value="
                            onFilterChange('teacher', teacherFilter)
                            ">
                          <template v-slot:append>
                            <q-icon v-if="teacherFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                              teacherFilter = null;
                      ...
                      <div class="filter-item-wrapper" v-if="showDegreeFilter">
                        <q-select outlined v-model="degreeFilter" dense :options="degreeOptions" option-label="name"
                          option-value="id" fill-input emit-value map-options use-input input-debounce="0"
                          class="filter-select" :placeholder="degreeFilter ? '' : 'Degree'" @update:model-value="
                            onFilterChange('degree', degreeFilter)
                            ">
                          <template v-slot:append>
                            <q-icon v-if="degreeFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                              degreeFilter = null;
                      ...
                      <!-- Institution Filter -->
                      <div class="filter-item-wrapper" v-if="showInstitutionFilter">
                        <q-select outlined v-model="institutionFilter" dense :options="institutionOptions"
                          option-label="name" option-value="id" fill-input emit-value map-options use-input
                          input-debounce="0" class="filter-select" :placeholder="institutionFilter ? '' : 'All Institution'
                            " @update:model-value="
                          ...
                          <template v-slot:append>
                            <q-icon v-if="institutionFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                      ...
                      <div class="filter-item-wrapper" v-if="showJobTypeFilter">
                        <q-select outlined v-model="jobTypeFilter" dense :options="jobTypeOptions" option-label="name"
                          option-value="id" fill-input emit-value map-options use-input input-debounce="0"
                          class="filter-select" :placeholder="jobTypeFilter ? '' : 'Job Type'" @update:model-value="
                            onFilterChange('jobType', jobTypeFilter)
                            ">
                          <template v-slot:append>
                            <q-icon v-if="jobTypeFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                              jobTypeFilter = null;
                      ...
                      <div class="filter-item-wrapper" v-if="showCourseFilter">
                        <q-select outlined v-model="courseFilter" dense :options="courseOptions" option-label="name"
                          option-value="id" fill-input emit-value map-options use-input input-debounce="0"
                          class="filter-select" :placeholder="courseFilter ? '' : 'All Courses'" @update:model-value="
                            onFilterChange('course', courseFilter)
                            ">

                          <template v-slot:append>
                            <q-icon v-if="courseFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                              courseFilter = null;
                      ...
                      <div class="filter-item-wrapper" v-if="showTypeFilter">
                        <q-select outlined v-model="typeFilter" dense :options="typeOptions" option-label="name"
                          option-value="id" fill-input emit-value map-options use-input input-debounce="0"
                          class="filter-select" :placeholder="typeFilter ? '' : typePlaceholder" @update:model-value="
                            onFilterChange('type', typeFilter)
                            ">

                          <template v-slot:append>
                            <q-icon v-if="typeFilter" name="cancel" class="cursor-pointer" @click.stop.prevent="
                              typeFilter = null;
                    ...
                    <div class="view-report cursor-pointer" v-if="transactions" @click="$emit('viewReport')">
                      View Report
                    </div>
                    <slot name="header-actions"></slot>

                    <q-btn-dropdown class="add-btn-header q-ml-auto" v-if="showAddButtonDropdown" no-caps flat
                      content-class="action-menu" :label="addBtnLabel">
                      <q-list>
                        <q-item v-for="option in addDropdownOptions" :key="option.name" clickable v-close-popup
                          @click="handleDropdownAction(option)" class="action-menu-item">
                ...
                <div>
                  <q-btn flat class="add-btn-header justify-end" @click="addNew" v-if="showAddButton">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                      <g clip-path="url(#clip0_2896_2347)">
                        <circle cx="9" cy="9" r="7.5" stroke="white" stroke-width="1.5" />
                        <path d="M11.25 9.00002L9 9.00002M9 9.00002L6.75 9.00002M9 9.00002L9 6.75M9 9.00002L9 11.25"
            ...
            <template v-slot:body-cell-image="props">
              <q-td :props="props">
                <div class="row items-center no-wrap">
                  <q-img class="image" :src="props.row.picture_thumb" ratio="1" />
            ...
            <template v-slot:body-cell-balanceDisplay="props">
              <q-td :props="props">
                <div class="row items-center no-wrap">
                  ...
                  <q-btn flat round dense class="" @click.stop="toggleBalance(props.row)">
                    <!-- Show Icon (Eye) - Shows when Hidden -->
                    <svg v-if="!isBalanceVisible(props.row)" width="16" height="16" viewBox="0 0 16 16" fill="none"
            ...
            <template v-slot:body-cell-score="props">
              <q-td :props="props">
                <slot name="body-cell-score" :row="props.row">
                  <q-input v-model="props.row.score" dense readonly outlined class="score-input" type="number" />
                </slot>
              </q-td>
            </template>
            ...
            <template v-slot:body-cell-type="props">
              <q-td :props="props">
                <div :class="props.row.typeClass" class="type-cell row items-center no-wrap">
                ...
                <q-btn flat dense round class="action-menu-btn" v-if="ShowActionsdropDown">
                  ...
                  <q-menu class="action-menu">
                    <q-list>
                      ...
                      <q-item v-if="transactions" clickable class="action-menu-item"
                        @click="$emit('viewDetails', props.row)">
                        <q-item-section>View Details</q-item-section>
                      </q-item>
                      <q-item v-if="transactions" clickable class="action-menu-item" @click="EditEvent(props.row)">
                      ...
                      <q-item v-if="transactions" clickable class="action-menu-item text-negative"
                        @click="DeleteEvent(props.row)">
                        <q-item-section>Delete</q-item-section>
                      </q-item>

                      <q-item v-if="student || staff || profiles" clickable class="action-menu-item"
                      ...
                      <q-item v-if="
                        ...
                        profiles ||
                        staff
                      " clickable class="action-menu-item" @click="details(props.row)">
                        <q-item-section>View</q-item-section>
                      </q-item>

                      <q-item v-if="studentCourse" clickable class="action-menu-item"
                        @click="openDialogDelete(props.row)">
                        <q-item-section>Delete</q-item-section>
                      </q-item>

                      <q-item v-if="studentCourse" clickable class="action-menu-item" @click="AddDiscount(props.row)">
            ...
            <template v-slot:no-data>
              <div class="full-width row flex-center q-pa-md">
                <empty-state :title="emptyStateTitle" :description="emptyStateDescription"
                  :buttonLabel="emptyStateButtonLabel" :showButton="showEmptyStateButton" @action-click="addNew" />
              </div>
            </template>
          </q-table>
        </div>
