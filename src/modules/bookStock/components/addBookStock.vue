    ...
    <q-card class="custom-popup">
      <q-card-section>
        ...
        <div class="form-group">
          ...
          <q-input v-model.number="form.quantity" type="number" outlined dense placeholder="0" class="custom-input" />
        </div>

        <div class="form-group">
          <label>Direction <span>*</span></label>
          <div class="row q-gutter-md">
            <q-radio v-model="form.direction" val="Stock In" label="Stock In" color="teal-10" dense />
            <q-radio v-model="form.direction" val="Stock Out" label="Stock Out" color="teal-10" dense />
          </div>
        </div>

        <div class="popup-actions justify-end">
          <q-btn label="Cancel" v-close-popup flat class="btn-cancel" no-caps />
          <q-btn label="Save & Add Another" class="btn-save-add" no-caps flat @click="save(false)" />
          <q-btn label="Save & Close" class="btn-save-close" no-caps flat @click="save(true)" />
        </div>
      </q-card-section>
    </q-card>
  </q-dialog>
...
const save = (close) => {
  if (!form.value.level || form.value.quantity <= 0) return;
