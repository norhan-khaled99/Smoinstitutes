    ...
    <template #body-cell-accountType="{ row }">
      <div class="row items-center no-wrap">
        <div
          ...
          :style="{
            backgroundColor: getAccountStyle(row.mainacctype_name).bgColor,
          }"
        >
          <span
            v-if="getAccountStyle(row.mainacctype_name).icon.startsWith('<svg')"
            v-html="getAccountStyle(row.mainacctype_name).icon"
            class="flex flex-center"
          ></span>
          <q-icon
            v-else
            :name="getAccountStyle(row.mainacctype_name).icon"
            size="20px"
            :class="getAccountStyle(row.mainacctype_name).textClass"
          />
        </div>
        <span>{{ row.mainacctype_name }}</span>
      </div>
    </template>
  </table-comp>
...
const columns = [
  ...
  },
  {
    ...
    label: "Account Type",
    align: "left",
    field: (row) => row.mainacctype_name,
    sortable: false,
  },
  {
    name: "mobile",
...
// Reset selected profile and edit mode when popup is closed
watch(showProfilePopup, (val) => {
  if (!val) {
