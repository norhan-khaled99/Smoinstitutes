    ...
    <q-card class="custom-popup">
      <q-card-section>
        ...
        <div class="form-group">
          ...
          <q-input v-model="noteText" type="textarea" outlined dense placeholder="Type the student note here..."
            class="custom-input" />
        </div>
        ...
        <div class="form-group">
          ...
          <div class="row q-gutter-md">
            <q-radio v-model="noteType" val="Information" label="Information" color="teal-10" dense />
            <q-radio v-model="noteType" val="Warning" label="Warning" color="teal-10" dense />
            <q-radio v-model="noteType" val="Important" label="Important" color="teal-10" dense />
          </div>
        </div>

        <div class="popup-actions justify-end">
          <q-btn label="Cancel" v-close-popup flat class="btn-cancel" />
          <q-btn label="Save & Add Another" class="btn-save-add" no-caps @click="saveNote(false)" />
          <q-btn label="Save & Close" class="btn-save-close" no-caps @click="saveNote(true)" />
        </div>
      </q-card-section>
    </q-card>
  </q-dialog>
...
const noteText = ref("");
const noteType = ref("Information");

const saveNote = (close) => {
  emit("save", {
    
  });

  noteText.value = "";
  noteType.value = "Information";
